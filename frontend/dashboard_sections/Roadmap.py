...
def render_roadmap():
    ...
    df["Normalized Quarter"] = df["Quarter"].apply(normalize_quarter)
    years = []
    ...
    for index, quarter in enumerate(quarters):
        with cols[index]:
            ...
            q_items = df[df["Normalized Quarter"] == quarter]
            if q_items.empty:
                short_quarter = quarter.split()[0]
                q_items = df[df["Normalized Quarter"] == short_quarter]
            ...
            for _, row in q_items.iterrows():
                with st.container(border=True):
                    ...
                    if pd.notna(row.get("Milestone")):
                        st.caption(f"Milestone: {row['Milestone']}")