...
def render_prioritized_initiatives():
    ...
    with st.container(border=True):
        ...
        st.text_area("Problem & Initiative Overview", value="Optimize database caching layer to reduce P95 analytical query latency.")
        ...
        if st.button("Calculate RICE & Save Priority", type="primary"):
            ...
            try:
                result = agent.execute({"title": title, "reach": reach, "impact": impact, "confidence": confidence, "effort": effort})
                st.success(f"Calculated RICE Score for **{title}**: **{result['score']}**")
                st.rerun()
            except ValueError as e:
                st.error(str(e))
    st.divider()
    st.subheader("Database Scored Initiatives")
    st.dataframe(fetch_initiatives_db(), use_container_width=True)