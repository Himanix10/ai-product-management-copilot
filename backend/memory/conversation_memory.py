...
class ConversationMemory:
    ...
    def get_history():
        ...
        try:
            rows = conn.execute(
                "SELECT user_message, assistant_response FROM chat_messages ORDER BY rowid ASC"
            ).fetchall()
        except Exception:
            rows = []
        finally:
    ...
    def add_message(role: str, content: str):
        ...
        try:
            if role == "user":
                ...
                cursor = conn.execute("SELECT rowid FROM chat_messages WHERE assistant_response IS NULL ORDER BY rowid DESC LIMIT 1")
                row = cursor.fetchone()
                if row:
                    conn.execute("UPDATE chat_messages SET assistant_response = ? WHERE rowid = ?", (content, row["rowid"]))
                else:
                    conn.execute(
